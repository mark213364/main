...
const BONUSES = {
  multiplier2: {
    ...
    desc: 'Удваивает клики на 1 минуту', price: 600,
    duration: 60 * 1000, multiplier: 2,
  },
};
...
async function handleReset(message, env) {
  ...
  await env.DB.prepare(
    'UPDATE counters SET count = 0, theme = "classic" WHERE chat_id = ?'
  ).bind(targetId).run();
  await env.DB.prepare(
    'DELETE FROM purchases WHERE chat_id = ?'
  ).bind(targetId).run();
...
async function handleApi(request, env, path, corsHeaders) {
  ...
  if (path === '/api/count' && request.method === 'POST') {
    let body;
    try { body = await request.json(); } catch { return json({ error: 'invalid body' }, 400, corsHeaders); }
  ...
  if (path === '/api/themes/buy' && request.method === 'POST') {
    ...
    return json({ ok: true, newCount: newCount, theme: theme }, 200, corsHeaders);
  }
